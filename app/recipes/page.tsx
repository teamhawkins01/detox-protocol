...
export default function RecipesPage() {
  ...
  useEffect(() => {
    const q = query(collection(db, 'recipes'), where('moderationStatus', '==', 'approved'));
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setMatchedRecipes(fetchedRecipes); // Initially show all
      setLoading(false);
    }, (err) => {
  ...
  const analyzeWithAI = async () => {
    ...
    try {
      ...
      // Prepare a lightweight version of recipes for the prompt to save tokens
      const recipeCatalog = recipes.map(r => ({
        id: r.id,
        title: r.title,
        category: r.category,
        benefits: r.ingredientBenefits?.join(', ') || '',
        outcome: r.outcome || ''
      }));
      ...
      const prompt = `
        ...
        Here is the catalog of available recipes:
        ${JSON.stringify(recipeCatalog)}
        
        Analyze the user's needs and select the top 1 to 6 most relevant recipes from the catalog.
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          responseSchema: {
            ...
            required: ["matchedRecipeIds", "analysis"]
          }
        }
      });
      ...
      if (data.matchedRecipeIds && data.matchedRecipeIds.length > 0) {
        const filtered = recipes.filter(r => data.matchedRecipeIds.includes(r.id));
        // Sort to match the AI's recommended order
        filtered.sort((a, b) => data.matchedRecipeIds.indexOf(a.id) - data.matchedRecipeIds.indexOf(b.id));
  ...
  const steps = [
    { id: 1, name: 'Goal', description: 'Primary Objective' },
    { id: 2, name: 'Symptoms', description: 'Current Issues' },
    { id: 3, name: 'Results', description: 'AI Formulation' }
  ];
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-8">
          <div>
            ...
            <h1 className="text-display text-5xl font-black text-white uppercase tracking-tighter leading-none">
              Protocol <span className="text-detox-green">Library</span>
            </h1>
          </div>
              ...
              >
                <div className="max-w-3xl">
                  <div className="flex items-center gap-3 text-detox-green mb-6">
                    ...
                    <span className="text-mono text-[10px] font-bold uppercase tracking-[0.3em]">Step 01 // Primary_Objective</span>
                  </div>
                  <h2 className="text-display text-4xl font-black text-white mb-6">What are you looking to achieve?</h2>
                  <p className="text-white/40 text-sm mb-12 leading-relaxed font-mono uppercase tracking-widest">
                    Select your primary health goal to help the Medicine Man filter the most effective clinical protocols for your needs.
                  </p>
            ...
            {currentStep === 2 && (
              ...
              >
                <div className="max-w-3xl">
                  <div className="flex items-center gap-3 text-detox-green mb-6">
                    <Filter className="w-6 h-6" />
                    <span className="text-mono text-[10px] font-bold uppercase tracking-[0.3em]">Step 02 // Current_Issues</span>
                  </div>
                  <h2 className="text-display text-4xl font-black text-white mb-6">Any specific symptoms?</h2>
                  <p className="text-white/40 text-sm mb-12 leading-relaxed font-mono uppercase tracking-widest">
                    Select any symptoms or issues you are currently experiencing. You can select multiple.
                  </p>
              ...
              >
                {isAnalyzing ? (
                  <div className="industrial-card p-20 bg-detox-gray border-detox-green/20 flex flex-col items-center justify-center text-center">
                    ...
                    <h2 className="text-display text-3xl font-black text-white mb-4">Medicine Man Analyzing...</h2>
                    <p className="text-white/40 text-sm font-mono uppercase tracking-widest max-w-md">
                      Cross-referencing your profile with our clinical database to find the optimal detox protocols.
                    </p>
                  </div>
                ) : (
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-8">
                        <h2 className="text-mono text-sm font-bold text-white/60 uppercase tracking-widest">
                          {matchedRecipes.length} Protocols Found
                        </h2>
                      </div>
                      ...
                      {matchedRecipes.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                          {matchedRecipes.map((recipe) => (
                            <ProtocolCard key={recipe.id} protocol={recipe} />
                          ))}
                        </div>
                      ) : (
